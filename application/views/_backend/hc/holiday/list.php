        ...
        <div class="card-body">
            ...
            <div class="card-title">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <a name="" id="" class="btn btn-primary" style="float: right" href="<?=base_url()?>hc/holiday/add" role="button">
            ...
            <table id="datatable" class="table table-bordered dt-responsive w-100">
                ...
                <tbody>
                    <?php
                    foreach ($data as $key) {
                        $getData = json_decode($this->api->CallAPI('GET', human_capital_api('/api/v1/Holiday'), ['HolidayID' => $key->HolidayID]))->result;
                    ?>
                    <tr>
                        <td>
                            <span class="badge badge-pill badge-soft-info font-size-11"><?=$key->HolidayDateCreated?></span>
                            <span class="badge badge-pill badge-soft-success font-size-11">
                                <i class="bx bx-check"></i> <?=$key->HolidayTenant?>
                            </span>
                            <br/>
                            <?=$key->HolidayName?>
                        </td>
                        <td>
                            <span class="badge badge-pill badge-soft-primary font-size-11">
                                <i class="bx bx-time"></i> <?=$key->HolidayNumber?> <?=lang('day_off')?>
                            </span>
                        </td>
                        <td>
                            <a name="" id="" class="btn btn-primary" href="<?=base_url()?>hc/holiday/update/<?=$key->HolidayID?>" role="button">
                                <i class="fa fa-edit"></i>
                                <?=lang('ubah')?>
                            </a>
                            <a name="" id="" class="btn btn-danger" href="<?=base_url()?>hc/holiday/delete/<?=$key->HolidayID?>" role="button">
