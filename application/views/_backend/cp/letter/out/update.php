    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="row card-body">
                <div class="col-md-6">
                    <?=form_open_multipart($this->uri->segment(1).'/letter/updateProses/'.$type.'/'.$id)?>
                    <div class="mb-3">
                        <label class="form-label"><?=lang('letter_no')?></label>
                        <input type="text" class="form-control" id="LetterNo" name="LetterNo" value="<?=$data->LetterNo?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label"><?=lang('letter_name')?></label>
                        <input type="text" class="form-control" id="LetterName" name="LetterName" value="<?=$data->LetterName?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label"><?=lang('document_number')?></label>
                        <input type="text" class="form-control" id="DocumentNumber" name="DocumentNumber" value="<?=$data->DocumentNumber?>">
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="LetterSecrecy" id="LetterSecrecy" class="form-control">
                            <?php for ($i = 0; $i < count($arr); $i++){ ?>
                                <option value="<?=$arr[$i]?>" <?=($arr[$i] == $data->LetterSecrecy)?'selected':''?>><?=$arr[$i]?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label"><?=lang('letter_date')?></label>
                        <input type="date" class="form-control" id="LetterDate" name="LetterDate" value="<?=date_format(date_create($data->LetterDate), "Y-m-d")?>">
                    </div>
